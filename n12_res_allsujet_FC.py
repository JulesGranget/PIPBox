...
def plot_allsujet_FC_time_stretch():
    ...
    for fc_metric in ['MI', 'ISPC', 'WPLI']:
        ...
        if fc_metric == 'MI':
            ...
            fc_allsujet_rscore = np.zeros((len(sujet_list_FC), len(pairs_to_compute), stretch_point_FC))
        else:
            fc_allsujet = np.zeros((len(sujet_list_FC), len(pairs_to_compute), len(freq_band_fc_list), stretch_point_FC))
            fc_allsujet_rscore = np.zeros((len(sujet_list_FC), len(pairs_to_compute), len(freq_band_fc_list), stretch_point_FC))
        ...
        for sujet_i, sujet in enumerate(sujet_list_FC):
            ...
            if fc_metric == 'MI':
                ...
                _fc_sujet_rscore = xr.open_dataarray(f'{fc_metric}_stretch_{sujet}_rscore.nc')

                _fc_sujet = _fc_sujet.loc['CHARGE'] - _fc_sujet.loc['VS']
                _fc_sujet_rscore = _fc_sujet_rscore.loc['CHARGE'] - _fc_sujet_rscore.loc['VS']

                _fc_sujet = _fc_sujet.median('ntrials')
                _fc_sujet_rscore = _fc_sujet_rscore.median('ntrials')
            
            else:
                _fc_sujet = xr.open_dataarray(f'{fc_metric}_{sujet}_stretch.nc')
                _fc_sujet_rscore = xr.open_dataarray(f'{fc_metric}_{sujet}_stretch_rscore.nc')

                _fc_sujet = _fc_sujet.loc[:, 'CHARGE'] - _fc_sujet.loc[:, 'VS']
                _fc_sujet_rscore = _fc_sujet_rscore.loc[:, 'CHARGE'] - _fc_sujet_rscore.loc[:, 'VS']

                _fc_sujet = _fc_sujet.median('cycle')
                _fc_sujet_rscore = _fc_sujet_rscore.median('cycle')

            fc_allsujet[sujet_i] = _fc_sujet
            fc_allsujet_rscore[sujet_i] = _fc_sujet_rscore
        ...
        fc_allsujet_rscore = xr.DataArray(data=fc_allsujet_rscore, dims=fc_allsujet_dict.keys(), coords=fc_allsujet_dict.values())
        ...
        for band in freq_band_fc_list:
            ...
            for pair_i, pair in enumerate(pairs_to_compute):
                ...
                for sujet_i, sujet in enumerate(sujet_list_FC):
                    ...
                    if fc_metric == 'MI':
                        ax.plot(time_vec, fc_allsujet_rscore.loc[sujet, pair], alpha=0.2)
                    else:
                        ax.plot(time_vec, fc_allsujet_rscore.loc[sujet, pair, band], alpha=0.2)

                if fc_metric == 'MI':
                    ax.plot(time_vec, fc_allsujet_rscore.loc[:, pair].median('sujet'), color='r')
                else:
                    ax.plot(time_vec, fc_allsujet_rscore.loc[:, pair, band].median('sujet'), color='r')
...
def plot_allsujet_FC_mat_stretch():
    ...
    for fc_metric in ['MI', 'ISPC', 'WPLI']:
        ...
        shifted_fc_allsujet_rscore = fc_allsujet_rscore.roll(time=-phase_shift, roll_coords=False)
        ...
        for band_i, band in enumerate(freq_band_fc_list):
            ...
            #### whole
            if fc_metric == 'MI':
                fc_mat_mask_signi_whole = from_pairs_2mat(clusters.loc['whole',:], pairs_to_compute)
            else:
                fc_mat_mask_signi_whole = from_pairs_2mat(clusters.loc['whole',band,:], pairs_to_compute)
            ...
            for pair_i, pair in enumerate(pairs_to_compute):
                ...
                if fc_metric == 'MI' and clusters.loc['whole',pair].values.astype('bool'):
                    fc_mat_only_signi_whole[A_i, B_i], fc_mat_only_signi_whole[B_i, A_i] = fc_val, fc_val
                elif fc_metric != 'MI' and clusters.loc['whole',band,pair].values.astype('bool'):
                    fc_mat_only_signi_whole[A_i, B_i], fc_mat_only_signi_whole[B_i, A_i] = fc_val, fc_val
            ...
            for phase_i, phase in enumerate(phase_list):
                ...
                if fc_metric == 'MI':
                    fc_mat_mask_signi_phase[phase_i,:,:] = from_pairs_2mat(clusters.loc[phase,:], pairs_to_compute)
                else:
                    fc_mat_mask_signi_phase[phase_i,:,:] = from_pairs_2mat(clusters.loc[phase,band,:], pairs_to_compute)
                ...
                for pair_i, pair in enumerate(pairs_to_compute):
                    ...
                    if fc_metric == 'MI':
                        data_chunk_diff = shifted_fc_allsujet_rscore.loc[:, pair, phase_vec[phase]].median('sujet').values
                        
                    else:
                        data_chunk_diff = shifted_fc_allsujet_rscore.loc[:, pair, band, phase_vec[phase]].median('sujet').values
            ...
            for fc_type in ['fullmat', 'signimat']:
                ...
                if fc_metric == 'MI':
                    plt.savefig(f'whole_stretch_MI_FC_{fc_type}.jpeg', dpi=150)

                else:
                    plt.savefig(f'whole_stretch_{fc_metric}_{band}_FC_{fc_type}.jpeg', dpi=150)
                ...
                if fc_metric == 'MI':
                    plt.savefig(f'phase_stretch_MI_FC_{fc_type}.jpeg', dpi=150)

                else:
                    plt.savefig(f'phase_stretch_{fc_metric}_{band}_FC_{fc_type}.jpeg', dpi=150)
                ...
                if fc_type == 'signimat':
                    ...
                    if fc_metric == 'MI':
                        plt.savefig(f'whole_stretch_GRAPH_MI_FC_{fc_type}.jpeg', dpi=150)

                    else:
                        plt.savefig(f'whole_stretch_GRAPH_{fc_metric}_{band}_FC_{fc_type}.jpeg', dpi=150)
                    ...
                    if fc_metric == 'MI':
                        plt.savefig(f'phase_stretch_GRAPH_MI_FC_{fc_type}.jpeg', dpi=150)

                    else:
                        plt.savefig(f'phase_stretch_GRAPH_{fc_metric}_{band}_FC_{fc_type}.jpeg', dpi=150)
        ...
        for band_i, band in enumerate(freq_band_fc_list):
            ...
            for metric in ['degree', 'clustering_coeff', 'betweenness', 'eigenvector']:
                ...
                for mode in ['mask', 'percentile']: 
                    ...
                    if fc_metric == 'MI':
                        plt.suptitle(f"{metric} mode:{mode}")
                    else :
                        plt.suptitle(f"{metric} {band} mode:{mode}")

                    # plt.show()

                    if fc_metric == 'MI':
                        plt.savefig(f"NODES_stretch_{fc_metric}_{mode}_graph_{metric}.png")
                    else:
                        plt.savefig(f"NODES_stretch_{fc_metric}_{band}_{mode}_graph_{metric}.png")
            ...
            for mode in ['mask', 'percentile']: 
                ...
                if fc_metric == 'MI':
                    ...
                    plt.suptitle(f"{band} mode:{mode}")

                # plt.show()

                if fc_metric == 'MI':
                    plt.savefig(f"GRAPHS_stretch_{fc_metric}_{mode}_graph.png")
                else:
                    plt.savefig(f"GRAPHS_stretch_{fc_metric}_{band}_{mode}_graph.png")
