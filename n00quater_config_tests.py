...
def test_FC_metrics():
    ...
    def generate_synchronized_signals(duration_tot=1200, num_windows=50, window_dur=5, noise_coeff=1, freq_sync=10, srate=500, nshift=1000, amp_coeff=2):
        ...
        for start in sync_windows:
            ...
            osc = np.sin(2 * np.pi * freq_sync * np.arange(0, window_dur, 1/srate))
    ...
    signal1, signal2, sync_windows = generate_synchronized_signals(duration_tot=duration_tot, num_windows=num_windows, window_dur=window_dur, 
                                                                   srate=srate, nshift=nshift, freq_sync=freq_sync, amp_coeff=amp_coeff)
    ...
    res_win_size_fc = np.zeros((3, np.arange(2,30,4).size, (window_dur+pre_win+post_win)*srate))
    ...
    win_conv = int(ncycle_FC/freq_sync*srate)
    pre_win, post_win = 4, 4
    res_noise_coeff_fc = np.zeros((3, noise_vec.size, (window_dur+pre_win+post_win)*srate))
