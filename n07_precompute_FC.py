...
def get_MI_sujet_stretch(sujet):
    ...
    #### verify computation
    if os.path.exists(os.path.join(path_precompute, 'FC', f'MI_{sujet}_stretch.nc')):
...
def get_ISPC_WPLI_stretch(sujet):
    ...
    if os.path.exists(os.path.join(path_precompute, 'FC', 'ISPC', f'ISPC_{sujet}_stretch.nc')) and os.path.exists(os.path.join(path_precompute, 'FC', 'WPLI', f'WPLI_{sujet}_stretch.nc')):
        print(f'ALREADY DONE')
        return
    ...
    for i in range(len(chan_list_eeg_short)):
        for j in range(i + 1, len(chan_list_eeg_short)):
            pairs_to_compute.append(f'{chan_list_eeg_short[i]}-{chan_list_eeg_short[j]}')
    
    ISPC_sujet = np.zeros((len(cond_sel), len(freq_band_fc_list), len(pairs_to_compute), nrespcycle_FC, stretch_point_FC))
    WPLI_sujet = np.zeros((len(cond_sel), len(freq_band_fc_list), len(pairs_to_compute), nrespcycle_FC, stretch_point_FC))
    
    def compute_ISPC_WPLI(band_i):

        band = freq_band_fc_list[band_i]
        ISPC_band = np.zeros((len(cond_sel), len(pairs_to_compute), nrespcycle_FC, stretch_point_FC))
        WPLI_band = np.zeros((len(cond_sel), len(pairs_to_compute), nrespcycle_FC, stretch_point_FC))
        ...
        for cond_i, cond in enumerate(cond_sel):
            ...
            convolutions = np.array([
                ...
                for ch in range(len(chan_list_eeg_short))
            ])
            ...
            for pair in pairs_to_compute:
                ...
                x, y = convolutions[idx_A], convolutions[idx_B]
                x_pad, y_pad = np.pad(x, ((0, 0), (win_conv//2, win_conv//2)), mode='reflect'), np.pad(y, ((0, 0), (win_conv//2, win_conv//2)), mode='reflect')
                
                # ISPC_conv = np.random.rand(x.shape[0], x.shape[1])
                # WPLI_conv = np.random.rand(y.shape[0], y.shape[1])

                ISPC_conv = np.array([[get_ISPC_2sig(x_pad[w, i:i+win_conv], y_pad[w, i:i+win_conv]) for i in range(x.shape[1])] for w in range(x.shape[0])])
                WPLI_conv = np.array([[get_WPLI_2sig(x_pad[w, i:i+win_conv], y_pad[w, i:i+win_conv]) for i in range(x.shape[1])] for w in range(x.shape[0])])

                #### median over wavelets
                ISPC_conv = np.median(ISPC_conv, axis=0)
                WPLI_conv = np.median(WPLI_conv, axis=0)
                
                ISPC_stretch = stretch_data(respfeatures_sujet[cond], stretch_point_FC, ISPC_conv, srate)[0]
                WPLI_stretch = stretch_data(respfeatures_sujet[cond], stretch_point_FC, WPLI_conv, srate)[0]

                #### cap ntrials
                ISPC_stretch = ISPC_stretch[:nrespcycle_FC,:]
                WPLI_stretch = WPLI_stretch[:nrespcycle_FC,:]

                if debug:

                    plt.pcolormesh(ISPC_stretch)
                    plt.show()

                    plt.pcolormesh(WPLI_stretch)
                    plt.show()
                
                ISPC_stretch_all.append(ISPC_stretch)
                WPLI_stretch_all.append(WPLI_stretch)
            
            ISPC_band[cond_i] = np.array(ISPC_stretch_all)
            WPLI_band[cond_i] = np.array(WPLI_stretch_all)
        
        return ISPC_band, WPLI_band
    
    results = joblib.Parallel(n_jobs=n_core, prefer='processes')(
        joblib.delayed(compute_ISPC_WPLI)(band_i) for band_i in range(len(freq_band_fc_list))
    )
    
    for band_i, (ISPC_res, WPLI_res) in enumerate(results):
        ISPC_sujet[:, band_i] = ISPC_res
        WPLI_sujet[:, band_i] = WPLI_res
    
    xr_dict = {'cond': cond_sel, 'band': freq_band_fc_list, 'pair': pairs_to_compute, 'cycle': np.arange(ncycle_FC), 'time': np.arange(stretch_point_FC)}
    ...
    os.chdir(os.path.join(path_precompute, 'FC', 'ISPC'))
    xr_ispc.to_netcdf(os.path.join(path_precompute, 'FC', 'ISPC', f'ISPC_{sujet}_stretch.nc'))

    os.chdir(os.path.join(path_precompute, 'FC', 'WPLI'))
    xr_wpli.to_netcdf(os.path.join(path_precompute, 'FC', 'WPLI', f'WPLI_{sujet}_stretch.nc'))
...
if __name__ == '__main__':
    ...
    execute_function_in_slurm_bash('n07_precompute_FC', 'get_ISPC_WPLI_stretch', [[sujet] for sujet in sujet_list_FC], n_core=15, mem='20G')
    #sync_folders__push_to_crnldata()
