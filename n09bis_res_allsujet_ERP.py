            ...
            for nchan_i, nchan in enumerate(chan_list_eeg):
                ...
                for cond_i, cond in enumerate(cond_list):
                    ...
                    for start_i, start_time in enumerate(inspi_starts):
                        ...
                        # data_ERP[start_i, :] = (x_chunk - x_mean) / x_std
                    ...
                    xr_data.loc[sujet, cond, nchan, :] = data_stretch_clean.mean(axis=0)
                    xr_data_sem.loc[sujet, cond, nchan, :] = data_stretch_clean.std(axis=0) / np.sqrt(data_stretch_clean.shape[0])
            ...
            for nchan_i, nchan in enumerate(chan_list_eeg):
                ...
                for cond_i, cond in enumerate(cond_list):
                    ...
                    if debug:
                        ...
                        data_stretch_respi, mean_inspi_ratio = stretch_data(respfeatures[cond], stretch_point_ERP, zscore(respi), srate)

                        for inspi_i in range(data_stretch_respi.shape[0]):

                            plt.plot(np.arange(stretch_point_ERP), data_stretch_respi[inspi_i, :], alpha=0.3)

                        plt.vlines(stretch_point_ERP/2, ymax=data_stretch_respi.max(), ymin=data_stretch_respi.min(), color='k')
                        plt.plot(np.arange(stretch_point_ERP), data_stretch_respi.mean(axis=0), color='r')
                        plt.title(f'{cond} : {data_stretch_respi.shape[0]}')
                        plt.show()

                        plt.plot(np.hstack((data_stretch_respi.mean(axis=0)[int(stretch_point_ERP/2):], data_stretch_respi.mean(axis=0)[:int(stretch_point_ERP/2)])))
                        plt.show()
...
def plot_ERP_diff(stretch=False):
    ...
    for nchan_i, nchan in enumerate(chan_list_eeg):
        ...
        #### save
        os.chdir(os.path.join(path_results, 'ERP', 'summary_diff', 'allchan'))
        ...
        if nchan in chan_list_eeg_short:
            ...
            os.chdir(os.path.join(path_results, 'ERP', 'summary_diff'))
...
def get_cluster_stats_manual_prem(stretch=False):
    ...
    if stretch and os.path.exists(os.path.join(path_precompute, 'ERP', 'cluster_stats_stretch.nc')):

        os.chdir(os.path.join(path_precompute, 'ERP'))

        print('ALREADY COMPUTED', flush=True)

        cluster_stats = xr.open_dataarray('cluster_stats_stretch.nc')

        return cluster_stats
        
    elif stretch == False and os.path.exists(os.path.join(path_precompute, 'ERP', 'cluster_stats.nc')):

        os.chdir(os.path.join(path_precompute, 'ERP'))

        print('ALREADY COMPUTED', flush=True)

        cluster_stats = xr.open_dataarray('cluster_stats.nc')
        ...
        for nchan_i, nchan in enumerate(chan_list_eeg):
            ...
            data_baseline = xr_data.loc[:, 'VS', nchan, :].values
            data_cond = xr_data.loc[:, 'CHARGE', nchan, :].values
        ...
        if stretch:
            xr_cluster.to_netcdf('cluster_stats_stretch.nc')
        else:
            xr_cluster.to_netcdf('cluster_stats.nc')

        return xr_cluster



def get_cluster_stats_manual_prem_one_cond_stretch():

    ######## VERIFY COMPUTATION ########
    if os.path.exists(os.path.join(path_precompute, 'ERP', 'cluster_stats_manual_perm_one_cond_stretch.nc')):

        os.chdir(os.path.join(path_precompute, 'ERP'))

        print('ALREADY COMPUTED', flush=True)
        xr_cluster_based_perm = xr.open_dataarray('cluster_stats_manual_perm_one_cond_stretch.nc')

    ######## COMPUTE ########
    else:

        time_vec = np.arange(stretch_point_ERP)
        xr_coords = {'sujet' : sujet_list, 'cond' : cond_list, 'chan' : chan_list_eeg, 'time' : time_vec}

        ######## INITIATE MEMMAP ########
        os.chdir(path_memmap)
        xr_data = np.memmap(f'cluster_based_perm_one_cond_stretch.dat', dtype=np.float32, mode='w+', shape=(len(sujet_list), len(cond_list), len(chan_list_eeg), time_vec.size))

        ######## PARALLELIZATION FUNCTION ########
        #sujet_i, sujet = 0, sujet_list[0]
        def get_cluster_based_perm_stretch_one_sujet(sujet_i, sujet):
        # for sujet_i, sujet in enumerate(sujet_list):
            ...
            for cond_i, cond in enumerate(cond_list):
                ...
                for nchan_i, nchan in enumerate(chan_list_eeg):
                    ...
                    #### load
                    x = load_data_sujet(sujet, cond)[nchan_i,:]
                    ...
                    data_stretch_respi, mean_inspi_ratio = stretch_data(respfeatures[cond], stretch_point_ERP, x, srate)

                    erp_data[cond][nchan] = data_stretch_respi
       
            ######## COMPUTE PERMUTATION ########
            print(f'{sujet} COMPUTE PERMUTATION')

            #cond_i, cond = 1, cond_list[1]
            for cond_i, cond in enumerate(cond_list):
                ...
                for chan_i, nchan in enumerate(chan_list_eeg):
                    ...
                    data_cond = erp_data[cond][nchan]
                    respfeatures_stretch = respfeatures[cond]

                    mask = get_permutation_cluster_1d_stretch_one_cond(data_cond, x, respfeatures_stretch, ERP_n_surrogate)

                    xr_data[sujet_i, cond_i, chan_i, :] = mask

            print(f'{sujet} done')

        ######## PARALLELIZATION COMPUTATION ########
        joblib.Parallel(n_jobs = n_core, prefer = 'processes')(joblib.delayed(get_cluster_based_perm_stretch_one_sujet)(sujet_i, sujet) for sujet_i, sujet in enumerate(sujet_list))

        ######## SAVE ########
        xr_cluster_based_perm = xr.DataArray(data=xr_data, dims=xr_coords.keys(), coords=xr_coords.values())

        os.chdir(os.path.join(path_precompute, 'ERP'))
        xr_cluster_based_perm.to_netcdf('cluster_stats_manual_perm_one_cond_stretch.nc')

        os.chdir(path_memmap)
        try:
            os.remove(f'cluster_stats_manual_perm_one_cond_stretch.dat')
            del xr_data
        except:
            pass
...
def get_cluster_stats_manual_prem_one_cond():

    ######## VERIFY COMPUTATION ########
    if os.path.exists(os.path.join(path_precompute, 'ERP', 'cluster_stats_manual_perm_one_cond.nc')):

        os.chdir(os.path.join(path_precompute, 'ERP'))

        print('ALREADY COMPUTED', flush=True)
        xr_cluster_based_perm = xr.open_dataarray('cluster_stats_manual_perm_one_cond.nc')
    ...
    ######## COMPUTE ########
        ...
        ######## INITIATE MEMMAP ########
        os.chdir(path_memmap)
        xr_data = np.memmap(f'cluster_based_perm_one_cond.dat', dtype=np.float32, mode='w+', shape=(len(sujet_list), len(cond_list), len(chan_list_eeg), time_vec.size))
        ...
        # for sujet_i, sujet in enumerate(sujet_list):
            ...
            for cond_i, cond in enumerate(cond_list):
                ...
                for nchan_i, nchan in enumerate(chan_list_eeg):
                    ...
                    #### chunk
                    point_ERP = int(np.abs(ERP_time_vec[0])*srate + ERP_time_vec[1]*srate)
                    data_ERP = np.zeros((inspi_starts.shape[0], int(point_ERP)))
                    ...
                    #### clean
                    data_ERP_clean = data_ERP[~((data_ERP >= 3) | (data_ERP <= -3)).any(axis=1),:]

                    erp_data[cond][nchan] = data_ERP_clean
            ...
            for cond_i, cond in enumerate(cond_list):
                ...
                for chan_i, nchan in enumerate(chan_list_eeg):
                    ...
                    mask = get_permutation_cluster_1d_one_cond(data_cond, x, ERP_n_surrogate)
        ...
        os.chdir(os.path.join(path_precompute, 'ERP'))
        xr_cluster_based_perm.to_netcdf('cluster_stats_manual_perm_one_cond.nc')

        os.chdir(path_memmap)
        try:
            os.remove(f'cluster_based_perm_one_cond.dat')
            del xr_data
        except:
            pass
